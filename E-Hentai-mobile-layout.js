...
function useLog() {
    ...
    const error = (message) => console.error(prefix, message);
    return {
        info,
        warn,
        error
...
function useUtils() {
    ...
    function addHomePagePreviewButton() {
        ...
        tbody.querySelectorAll("tr").forEach((tr) => {
            var _a;
            const glthumbId = (_a = tr.querySelector("div.glthumb")) === null || _a === void 0 ? void 0 : _a.id;
            if (!glthumbId)
                return;
            ...
            button.onclick = () => {
                const glthumbElem = document.querySelector(`div.glthumb#${glthumbId}`);
                if (!glthumbElem)
                    return;
                // 创建dialog
                ...
                // 克隆glthumb内容到dialog
                const content = glthumbElem.cloneNode(true);
                content.style.cssText = "display: block; width: 100%";
                content.firstChild.style.visibility = "visible";
